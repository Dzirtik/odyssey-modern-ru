...
import { buildBookAudioScript } from "./audio/books.mjs";
import { groupBookAudioSegments } from "./audio/book-one.mjs";

const execFileAsync = promisify(execFile);
const root = process.cwd();
const dryRun = process.argv.includes("--dry-run");
const generateAll = process.argv.includes("--all");
const selectedBookArgument = process.argv.find((argument) =>
  argument.startsWith("--book="),
);
const selectedBook = Number(selectedBookArgument?.split("=")[1] ?? 1);
const requestedBooks = generateAll
  ? Array.from({ length: 24 }, (_, index) => index + 1)
  : [selectedBook];
const concurrency = Math.max(
  1,
  Math.min(6, Number(process.env.AUDIO_CONCURRENCY ?? 4)),
);
const outputRoot = path.join(root, "public", "audio", "books");
const manifestRoot = path.join(root, "src", "data", "audio");
const exactReadingInstruction =
  " Прочитай весь предоставленный текст точно, ничего не добавляй, не сокращай и не пересказывай.";

if (
  requestedBooks.some(
    (book) => !Number.isInteger(book) || book < 1 || book > 24,
  )
) {
  throw new Error("Book number must be an integer from 1 to 24");
}
...
async function renderBook(bookNumber, apiKey) {
  const number = String(bookNumber).padStart(2, "0");
  const cacheRoot = path.join(root, ".audio-cache", "openai", `book-${number}`);
  const rawRoot = path.join(cacheRoot, "raw");
  const normalizedRoot = path.join(cacheRoot, "normalized");
  const outputAudio = path.join(outputRoot, `book-${number}.mp3`);
  const outputManifest = path.join(manifestRoot, `book-${number}.json`);
  const audioScript = await buildBookAudioScript(bookNumber);
  const groups = groupBookAudioSegments(
    audioScript.segments,
    bookNumber === 1 ? 2_800 : 2_600,
  );
  const summary = {
  ...
  const synthesize = async (group, index) => {
    ...
    if (await exists(file)) {
      console.log(
        `↻ ${number} · ${index + 1}/${groups.length} ${group.speakerLabel} (cache)`,
      );
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
      ...
      if (response.ok) {
        ...
        console.log(
          `✓ ${number} · ${index + 1}/${groups.length} ${group.speakerLabel}`,
        );
        return { ...group, rawFile: file, fingerprint };
      }
  ...
  const rendered = await mapWithConcurrency(groups, concurrency, synthesize);
...
if (!dryRun && !apiKey) throw new Error("OPENAI_API_KEY is not set");

const results = [];
for (const bookNumber of requestedBooks) {
  results.push(await renderBook(bookNumber, apiKey));
}
  ...
  JSON.stringify(
    {
      ...
      minutes: dryRun
        ? null
        : results.reduce((sum, result) => sum + result.duration, 0) / 60,
    },
    null,
    2,
  ),
